# added resizable bar but text is flickering
...
class ResizableCaptions:
    def __init__(self):
        ...
        # Color scheme
        self.bg_color = "#1a1a1a"  # Dark gray
        self.text_color = "#00ff9d"  # Neon cyan-green
        self.handle_color = "#ff69b4"  # Hot pink
        self.border_color = "#404040"  # Medium gray
    ...
    def create_widgets(self):
        ...
        self.canvas.create_rectangle(
            1, 1, 
            self.default_width-1, self.default_height-1,
    ...
    def create_resize_handles(self):
        ...
        # Corner handle
        self.corner_handle = tk.Frame(
            self.root,
            bg=self.handle_color,
            width=handle_size,
            height=handle_size
        )
        self.corner_handle.place(
            relx=1.0, 
            rely=1.0, 
            anchor='se',
            x=-1,
    ...
    def setup_bindings(self):
        ...
        self.corner_handle.bind("<ButtonPress-1>", lambda e: self.start_resize('corner'))
        self.corner_handle.bind("<B1-Motion>", lambda e: self.on_resize('corner'))
        ...
        try:
            with self.mic.recorder(
                ...
                while self.running:
                    ...
                    data = np.clip(data * 1.5, -1.0, 1.0)  # Boost audio levels
    ...
    def update_gui(self):
        """Update text display with smooth transitions"""
        try:
            if not self.transcript_queue.empty():
                ...
                display_text = self.text_buffer[-150:].replace("...", "")
                
                # Animate text update
                self.label.config(text=display_text)
                current_color = self.label.cget("fg")
                self.label.config(fg=self.bg_color)
                self.label.after(50, lambda: self.label.config(fg=current_color))
